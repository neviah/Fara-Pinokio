...
class MagenticOneBaseAgent(RoutedAgent):
    ...
    def log_name(self, suffix: str) -> str:
        return f"{self.metadata['type']}.{suffix}"

    async def _call_model_client(self, messages, model_client=None, **kwargs):
        if model_client is None:
            model_client = self._model_client

        response = await model_client.create(messages, **kwargs)

        if hasattr(self, "shared_data_point"):
            if self.shared_data_point:
                self.shared_data_point.add_token_usage(str(type(self).__name__), response.usage)

        if hasattr(self, "final_answer_store"):
            if self.final_answer_store:
                self.final_answer_store.add_token_usage(str(type(self).__name__), response.usage)
